...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (mysqli_query($conexion, $query)) {
    ...
    header("Location: ../Peliculas/carrito/mostrar_carrito.php"); // Redireccionar al carrito
...
<head>
  ...
  <link rel="stylesheet" href="../Peliculas/peliculas-detalle.css">
...
<header class="header">
  ...
  <nav>
    <ul class="nav-links">
      ...
      <li><a href="../Peliculas/carrito/mostrar_carrito.php">Carrito</a></li>
    </ul>
  </nav>
  </header>
  <div class="movie-details">
    ...
    <div class="movie-info">
      ...
      <form method="POST" action="../Peliculas/guardar_calificacion.php?id=<?php echo $id; ?>">
  <button class="like-button" name="cal" value="like">
  <i class="fas fa-thumbs-up fa-bounce"></i>
  </button>
  <button class="dislike-button" name="cal" value="dislike">
  <i class="fas fa-solid fa-thumbs-down fa-bounce" style="color: #fb2d2d;"></i>
  </button>
</form>
<form method="POST" action="../Peliculas/guardar_favoritos.php">
  <button class="favorite-button separation" name="id_peliculas" value="<?php echo $fila['id_peliculas']; ?>">
    ...
    <!-------------------------------------------------------------------------------------------->
      <div class="movie-price">
        <p>Precio: $<span id="precio"><?php echo $fila['precio']; ?></span> mxn</p>
        <p>Renta: $<span id="renta"><?php echo $fila['renta']; ?></span> mxn</p>
      </div>
      <form method="POST">
        <input type="hidden" name="id_peliculas" value="<?php echo $fila['id_peliculas']; ?>">
        <select name="modalidad" class="movie-info select-modalidad">
  ...
  <div class="movie-info-details">
    ...
    <p>Director: <?php echo $fila['descripcion']; ?></p>
...
function obtenerComentarios() {
  ...
  xhr.open('GET', '../Peliculas/obtener_comentario.php?id=<?php echo $id; ?>', true);
  xhr.onload = function () {
    if (xhr.status === 200) {
...
obtenerComentarios();
  ...
  comentarioForm.addEventListener('submit', function (e) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '../Peliculas/guardar_comentario.php?id=<?php echo $id; ?>', true);
